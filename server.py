...
def movement_portrayal(agent):
    ...
    portrayal = {"Shape": "rect", "Filled": "true", "w": 1, "h": 1, "Layer": 0}  # sets default settings for pixels

# higher layer number = drawn last, higher priority shown
# colors can be written as words or as hexademical codes; if the latter, must have a '#' in front, e.g. #ffffff
...
agent_slider = UserSettableParameter('slider', "Number of Families", 20, 1, 30, 1)
                                                                    # number params: default, min, max, interval
