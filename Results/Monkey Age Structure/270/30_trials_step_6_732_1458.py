...
This code assumes 3 sets of 30 trials and 1460 steps (20 years).
It will take each of the 30 trials' outputs for each setting.
...
Repeat this code for each setting, then again for each type of output file (human demographics, etc.).
"""

set_file_name = 'steps.csv'
file_name = 'abm_export_summary_monkeys'
...
for number in range(31, 61):
    read_first_line(file_name + str(number) + '.csv', 2)  # 0 years
for number in range(31, 61):
    read_first_line(file_name + str(number) + '.csv', 123)  # 10 years
for number in range(31, 61):
    read_first_line(file_name + str(number) + '.csv', 244)  # 20 years
